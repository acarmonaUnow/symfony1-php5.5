...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    {
      $file = sprintf('%s%s/%s/Base%s%s', $config['models_path'], isset($definition['package']) ? '/'.substr((string) $definition['package'], 0, strpos((string) $definition['package'], '.')) : '', $builderOptions['baseClassesDirectory'], $model, $builderOptions['suffix']);
